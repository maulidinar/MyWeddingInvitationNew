...
@section('content')
     ...
     <div class="content">
        ...
        <p>Upload Foto / Video secara parsial</p>
        ...
        <div class="grid grid-cols-12 gap-6 mt-5">
            <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
                ...
                @if (count($data) > 0)
                <a href="{{ URL::to('/kirim-foto-masal/'.$undangan->id) }}">
                    <button class="button text-white bg-theme-6 shadow-md mr-2">
                        Kirim Foto Masal
                    </button>
                </a>
                @endif
                ...
                <table class="table table-report -mt-2">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $key+1}}</td>
                            <td class="text-left">
                              <img src="{{ asset('storage/album/undangan/'.$undangan->id.'/'.$value->image) }}" alt="" width="150">
                            </td>
                            <td class="text-left">
                            ...
                            <td class="table-report__action w-auto ">
                                <div class="open-edit flex items-center mr-3"  style="float: right"> 
                                   <a data-id="{{ $value->id }}" data-undangan_id="{{ $undangan->id }}" class="open-AddBookDialog flex items-center text-theme-6" href="javascript:;"
                                        data-toggle="modal" data-target="#delete-confirmation-modal"> <i data-feather="trash-2" class="w-4 h-4 mr-1"></i>
                                        Delete </a>
                                </div>
        ...
        <div class="modal" id="delete-confirmation-modal">
            <div class="modal__content">
                <div class="p-5 text-center">
                    ...
                    <div class="text-3xl mt-5">Hapus Gambar ini</div>
                    <div class="text-gray-600 mt-2">Anda yakin ingin menghapus data ini.?
                        
                    </div>
                </div>
                <div class="px-5 pb-8 text-center">
                    <form action="{{ URL::to('gambar-destroy') }}" method="POST">   
                        <input type="hidden" name="id" id="val-id" value=""/>
                        <input type="hidden" name="undangan_id" id="undangan-id" value=""/>
                    @csrf
        ...
        <div class="modal" id="header-footer-modal-preview">
            <div class="modal__content">
                ...
                <div class="preview">
                    <form  action="{{ URL::to('/upload-gambar') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="intro-y box p-5">
                            <div>
                                <div class="mt-3"> <label>Pilih Nama Tamu</label>
                                    <div class="mt-2"> <select name="tamu_id" class="select2 block w-full border mt-2">
                                            <optgroup label="Daftar Tamu">
                            ...
                            <div>
                                <label>Gambar</label>
                                <input name="gambar" type="file" class="input w-full border mt-2" placeholder="Input alamat here" required>
                                <input name="undangan_id" type="hidden" value="{{ $undangan->id }}" class="input w-full border mt-2" placeholder="Input alamat here" required>
                            </div>
                            <div class="mt-3"></div>
                        </div>
    ...
    <script type="text/javascript">
        $(document).on("click", ".open-AddBookDialog", function () {
            ...
            var u_ud = $(this).data('undangan_id');
            console.log('id',val_id);
            $("#val-id").val( val_id );
            $("#undangan-id").val( u_ud );
        });
